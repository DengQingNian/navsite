...
const { getWolaiToken, getWolaiDatabase, addWolaiDatabaseRow } = require('./wolai');
moment.locale('zh-cn');
...
async function getWolaiAccessToken() {
  ...
  let resp = await getWolaiToken()
  cachedToken = resp.app_token
  tokenExpireTime = resp.expire_time == -1 ? new Date(2077, 0, 1) : new Date(resp.expire_time)
...
// 处理多维表格数据
function processTableData(items, isWolai) {
  // 提取记录并按分类分组
  const records = isWolai ? items : items.map(item => {
...
let feishuNavigationF = async (req, res) => {
  try {
    let data;
    let categories;
...
let wolaiNavigationF = async (req, res) => {
  try {
    let data;
    let categories;
    let isMockData = false;
    try {
      const token = await getWolaiAccessToken();
      const items = await getWolaiDatabase(token);
      data = processTableData(items, true);
      categories = Object.keys(data);
    } catch (apiError) {
      console.log('无法从飞书API获取数据，使用模拟数据:', apiError.message);
      // 使用模拟数据
...
function getNavigationF() {
  if (process.env.SOURCE == "wolai") {
    return wolaiNavigationF
  } else {
    return feishuNavigationF
  }
}


async function addFeishuLink(requestBody) {
  // 获取飞书访问令牌
  const token = await getTenantAccessToken();
...
//       }
//     }
async function addWolaiLink(requestBody) {
  const token = await getWolaiAccessToken()
  const createRecordBody = {
    "rows": [{
      "站点名称": requestBody.name,
      "网址": requestBody.url,
      "分类": requestBody.category,
      "排序": requestBody.sort || 0
    }]
  }
  addWolaiDatabaseRow(token, createRecordBody)
}

// API路由 - 获取导航数据
app.get('/api/navigation', getNavigationF());
...
app.post('/api/links', async (req, res) => {
  try {
    ...
    if (process.env.SOURCE == "wolai") {
      return await addWolaiLink(requestBody)
    } else {
      return await addFeishuLink(requestBody)
    }
